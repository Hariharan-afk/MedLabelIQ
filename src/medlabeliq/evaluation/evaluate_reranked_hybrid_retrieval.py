...
def heading_text(result: RerankedHybridSearchResult) -> str:
    return " > ".join(str(item) for item in result.heading_path)


def is_relevant(result: RerankedHybridSearchResult, case: EvalCase) -> bool:
...
def evaluate_case(case: EvalCase, top_k: int) -> dict[str, Any]:
    ...
    top_result: RerankedHybridSearchResult | None = results[0] if results else None
    ...
    return {
        ...
        "top_reranker_score": top_result.reranker_score if top_result else None,
        "top_original_hybrid_rank": (
            top_result.original_hybrid_rank if top_result else None
        ),
        "top_preview": (
            top_result.chunk_text[:300].replace("\n", " ")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate hybrid retrieval followed by reranking."
    )
    parser.add_argument(
        "--eval-set",
        default=str(
    ...
    parser.add_argument(
        "--output",
        default=str(
            settings.interim_dir / "reranked_hybrid_retrieval_eval_results.csv"
        ),
    )
...
def main() -> None:
    ...
    for row in rows:
        ...
        print(
            ...
            f"original_hybrid_rank={row['top_original_hybrid_rank']}"
        )
