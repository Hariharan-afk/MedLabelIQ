...
def heading_text(result: DenseSearchResult) -> str:
    return " > ".join(str(item) for item in result.heading_path)


def is_relevant(result: DenseSearchResult, case: EvalCase) -> bool:
...
def evaluate_case(case: EvalCase, top_k: int) -> dict[str, Any]:
    ...
    top_result: DenseSearchResult | None = results[0] if results else None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate dense Qdrant retrieval over section-aware chunks."
    )
    parser.add_argument(
        "--eval-set",
        default=str(settings.project_root / "data" / "evaluation" / "retrieval_eval_paraphrase.yaml"),
    )
    parser.add_argument(
        "--output",
        default=str(settings.interim_dir / "dense_retrieval_eval_results.csv"),
    )
