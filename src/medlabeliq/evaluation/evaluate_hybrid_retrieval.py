...
from medlabeliq.retrieval.hybrid_search import HybridSearchResult, hybrid_search_chunks
...
def heading_text(result: HybridSearchResult) -> str:
    return " > ".join(str(item) for item in result.heading_path)


def is_relevant(result: HybridSearchResult, case: EvalCase) -> bool:
...
def evaluate_case(case: EvalCase, top_k: int) -> dict[str, Any]:
    ...
    top_result: HybridSearchResult | None = results[0] if results else None
    ...
    return {
        ...
        "top_hybrid_score": top_result.hybrid_score if top_result else None,
        "top_lexical_rank": top_result.lexical_rank if top_result else None,
        "top_dense_rank": top_result.dense_rank if top_result else None,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate hybrid lexical+dense retrieval."
    )
    parser.add_argument(
        "--eval-set",
        default=str(
            ...
            / "data"
            / "evaluation"
            / "retrieval_eval_paraphrase.yaml"
        ),
    )
    parser.add_argument(
        "--output",
        default=str(settings.interim_dir / "hybrid_retrieval_eval_results.csv"),
    )
...
def main() -> None:
    ...
    for row in rows:
        ...
        print(
            ...
            f"lex_rank={row['top_lexical_rank']} | "
            f"dense_rank={row['top_dense_rank']}"
        )
